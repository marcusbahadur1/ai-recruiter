...
async def test_rag_scrape_small_firm_plan_accepted(client, mock_db, mock_tenant):
    ...
    with patch("app.services.rag_pipeline.scrape_website", AsyncMock(return_value=[])):
        resp = await client.post(
            "/api/v1/rag/scrape", json={"url": "https://example.com"}
        )
...
async def test_rag_scrape_returns_chunk_count(client, mock_db, mock_tenant):
    ...
    with patch(
        "app.services.rag_pipeline.scrape_website", AsyncMock(return_value=fake_docs)
    ):
...
async def test_rag_upload_txt_accepted(client, mock_db, mock_tenant):
    ...
    with patch(
        "app.services.rag_pipeline.upload_document", AsyncMock(return_value=[fake_doc])
    ):
        resp = await client.post(
            "/api/v1/rag/documents",
...
async def test_widget_chat_unknown_slug(client, mock_db):
    ...
    resp = await client.post(
        "/api/v1/widget/nonexistent/chat", json={"message": "Hello"}
    )
    assert resp.status_code == 404
...
async def test_widget_chat_returns_reply(client, mock_db, mock_tenant):
    ...
    with (
        patch(
            "app.services.rag_pipeline.query",
            AsyncMock(return_value=["Chunk about the firm"]),
        ),
        patch(
            "app.services.ai_provider.AIProvider.complete",
            AsyncMock(return_value="We help companies hire."),
        ),
    ):
        resp = await client.post(
            "/api/v1/widget/test-firm/chat",
            json={
...
async def test_widget_chat_with_history(client, mock_db):
    ...
    with (
        patch("app.services.rag_pipeline.query", AsyncMock(return_value=[])),
        patch(
            "app.services.ai_provider.AIProvider.complete",
            AsyncMock(return_value="Sure, we're hiring."),
        ),
    ):
        resp = await client.post(
            "/api/v1/widget/test-firm/chat",
