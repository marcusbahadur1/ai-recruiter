...
**MarketingAccount**: `tenant_id` (nullable), `platform="linkedin"`, `account_type=company|person`, `linkedin_urn`, `access_token/refresh_token` (encrypted Fernet), `token_expires_at`
...
```
GET /marketing/auth/linkedin → LinkedIn OAuth redirect
GET /marketing/auth/linkedin/callback?code=...
  → exchange code → encrypt tokens → store in MarketingAccount
```

**Pending**: LinkedIn MDP (Marketing Developer Platform) approval required for posting API. OAuth built; posting blocked until approved. See open questions.
